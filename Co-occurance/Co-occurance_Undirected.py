...
for token_compared in text_processed:
    for i, word in enumerate(token_compared):
        ...
        for j in range(i + 1, min(i + window_size + 1, len(token_compared))):
            w1, w2 = sorted([word, token_compared[j]])
            edge_dict[(w1, w2)] += 1

# Create edge dataframe
edges = []
min_weight = 10  # Minimum co-occurrences

for (w1, w2), weight in edge_dict.items():
    if weight >= min_weight:
        edges.append([w1, w2, weight])

edge_df = pd.DataFrame(edges, columns=['W1', 'W2', 'Weight'])
edge_df = edge_df.sort_values('Weight', ascending=False)

print(f"\nTotal edges: {len(edge_df)}")

# Save the result
edge_df.to_csv('Edge_Undirected.csv', index=False)

# Create and export .gexf for Gephi
graph = nx.Graph()

for _, row in edge_df.iterrows():
    graph.add_edge(row['W1'], row['W2'], weight=int(row['Weight']))

nx.write_gexf(graph, "Co-occurrence_Undirected.gexf")
